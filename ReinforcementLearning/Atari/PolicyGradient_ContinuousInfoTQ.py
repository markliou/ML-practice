...
def sQ(S, AH, SH, Li, reuse=False, trainable = True):
    with tf.variable_scope('sQ'):
        ...
        f1 = tf.layers.flatten(conv6b)
        f2 = tf.layers.dense(f1, 512, activation=tf.nn.elu, trainable = trainable, reuse=reuse, name='sf2')
        f3 = tf.layers.dense(f2, 256, activation=tf.nn.elu, trainable = trainable, reuse=reuse, name='sf3')
        ...
        fo = tf.layers.dense(f7, 32, activation=tf.nn.elu, trainable = trainable, reuse=reuse, name='sfo')
...
sAct_A = sQ(Act_S, Act_m, Sta_m, Act_clive, trainable = True, reuse=False) # give the current state, action history and state history
sAct_sample = tf.argmax(tf.nn.softmax(sAct_A), axis=-1)[0]

Q_weights = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='Q')
sQ_weights = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='sQ')
update_Q = [tf.assign(sq, q) for q,sq in zip(Q_weights,sQ_weights)]
...
# for episode in range(EPISODE):    
    episode += 1
    act_h = [ [0. for j in range(6)] for i in range(score_delay+action_memo)].copy()
    S_h = [ [ [ [0. for i in range(3)] for j in range(160)] for k in range(210)] for l in range(score_delay+action_memo)].copy()  # [None, 210, 160, 3]
    delay_CuReward = [0. for i in range(score_delay)].copy()
    ...
    # for step in range(STEP_LIMIT):
        ...
        # if False:
            ...
            A = sess.run(sAct_sample, feed_dict={
    ...
    OutFile.write("Epi:{}  Score:{}  Loss:{} Steps:{}\n".format(episode,Reward_cnt,Loss,steps))
