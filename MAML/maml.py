...
(kmnist_tr, kmnist_ts) = tfds.load('mnist', 
                                   split=['train','test'],
                                   shuffle_files=True,
                                   as_supervised=True,
                                   with_info=False,)
...
for step in range(5000):
    ...
    # calculating the outter loss
    loss_outter = lambda: tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=meta_labs, logits=cnn_model(imgs))) - \
                          tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=meta_labs, logits=cnn_model(imgs))) + \
                          outter_loss/inner_task_loop_no

    # loss_outter = lambda: outter_loss/inner_task_loop_no

    # update the meta-weights
    opt_outter.minimize(loss=loss_outter, var_list=cnn_model.trainable_weights) 

    print("step: {}  outter_loss: {}".format(step, loss_outter()))
pass 