...
def Spatial_attention(Q, K, compression_channel_no = 16, name = 'satt'):
    ...
    Q = tf.layers.conv2d(Q, compression_channel_no, [1,1], [1,1], padding="SAME", activation=tf.nn.elu, kernel_initializer=tf.keras.initializers.orthogonal(), name=name+'att_compressionQ')
    K = tf.layers.conv2d(K, compression_channel_no, [1,1], [1,1], padding="SAME", activation=tf.nn.elu, kernel_initializer=tf.keras.initializers.orthogonal(), name=name+'att_compressionK')
    
    Qf = tf.reshape(Q, [-1, Q.shape[1].value * Q.shape[2].value, compression_channel_no])
    Kf = tf.reshape(K, [-1, K.shape[1].value * K.shape[2].value, compression_channel_no])
    
    attention_map = tf.matmul(Qf, Kf, transpose_b=True)  # [bs, N, N]
    attention_map = tf.nn.softmax(attention_map, axis=1) # consider only the keys for attention
    
    attention = tf.matmul(attention_map, Kf)
