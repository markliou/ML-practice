#!/usr/bin/python3
###
# this version use the vector slicing for efficiently use the codes in codebook.
...
def VQVAE(X, act=tf.nn.relu, dic_size=128):
    with tf.variable_scope('vqvae_e'):
        conv1e = tf.keras.layers.Conv2D(32, [3, 3], strides=2, padding='SAME', activation=act)(X) # 14
        conv2e = tf.keras.layers.Conv2D(32, [3, 3], strides=2, padding='VALID', activation=act)(conv1e) # 7
        conv3e = tf.keras.layers.Conv2D(32, [3, 3], strides=2, padding='SAME', activation=act)(conv2e) # 3
