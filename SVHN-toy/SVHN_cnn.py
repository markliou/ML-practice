...
def stack_dataset(data_list, data):
    ...
    data_container = np.stack( [cv2.resize( ((cv2.imread(data[i]['addr'])-256)/256) , (size,size)) for i in data_list] )
...
def cnn(X):
    ...
    kernels = {
        ...
        'wc1': tf.Variable(initializer([7, 7, 3, 1024])),
        'wc2': tf.Variable(initializer([5, 5, 1024, 512])),
        'wc3': tf.Variable(initializer([5, 5, 512, 1024])),
        'wc4': tf.Variable(initializer([3, 3, 1024, 512])),
        'wc5': tf.Variable(initializer([3, 3, 512, 1024])),
    }
    fc = {
        'wf1': tf.Variable(initializer([1*1*1024, 1024])),
        'wf2': tf.Variable(initializer([1024,1024])),
        'out1': tf.Variable(initializer([1024,num_class])),
        'out2': tf.Variable(initializer([1024,num_class])),
        'out3': tf.Variable(initializer([1024,num_class])),
        'out4': tf.Variable(initializer([1024,num_class])),
        'out5': tf.Variable(initializer([1024,num_class])),
        'out6': tf.Variable(initializer([1024,num_class])),
    }
    bias = {
        'cb1': tf.Variable(initializer([1024])),
        'cb2': tf.Variable(initializer([512])),
        'cb3': tf.Variable(initializer([1024])),
        'cb4': tf.Variable(initializer([512])),
        'cb5': tf.Variable(initializer([1024])),
        'fb1': tf.Variable(initializer([1024])),
        'fb2': tf.Variable(initializer([1024])),
    ...
    # cl1 = tf.nn.relu(tf.nn.conv2d(X, kernels['wc1'], strides=[1, 1, 1, 1], padding='SAME') + bias['cb1'])
...
def main():
    ...
    logits = cnn(X)
    CE = tf.reduce_sum(tf.reduce_mean( tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits , labels=Y), axis=-1 ))
    ...
    with tf.Session() as sess:
        ...
        for step in range(opt_iteration):
            ...
            if step%10 == 0 :
                ...
                # show some results
                data_list = [ next(subdataset_g) for idxi in range(3)]
                data_container = stack_dataset(data_list, data)
                label_container = stack_labels(data_list, data)
